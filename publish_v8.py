...
for npz_path in files:
    ...
    # Fixed parameters — same for all cases, safe and asymptotic identical.
    # ds=2 → 1000³ grid (4 GB float32, fits in 64 GB RAM)
    # σ=1.0 → removes voxel jaggies without merging thin sheets
    ds = ds_override if ds_override else 4
    sig = smooth_sigma  # default 2.0 from CLI, but we override below
    sig = 1.0
    thr = 0.3
    sp = (h*ds, h*ds, h*ds)

    print(f"  ds={ds} ({N//ds}³), σ={sig:.2f}, thr={thr}")
