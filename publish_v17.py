...
def decimate_mesh(verts, faces, target):
    ...
    s.simplify_mesh(target_count=target, aggressiveness=5, verbose=0)
    v, f, _ = s.getMesh()
...
for npz_path in files:
    ...
    # ds=1 (full resolution), same for both meshes
    ds = ds_override if ds_override else 2
    sig = 0.5
    thr = 0.5
    sp = (h*ds, h*ds, h*ds)

    print(f"  ds={ds} ({N//ds}³), σ={sig}, thr={thr}")

    # === SAFE SET ===
    # Convert to float32, then free bool to save 8 GB
    Q_f = Q[::ds,::ds,::ds].astype(np.float32)
    del Q; gc.collect()
    # In-place smooth — avoids creating a second 32 GB array
    gaussian_filter(Q_f, sigma=sig, output=Q_f)
    gc.collect()
    vs,fs,ns,_ = marching_cubes(Q_f, level=thr, spacing=sp)
    ...
    Q_f = Q_asymp[::ds,::ds,::ds].astype(np.float32)
    del Q_asymp; gc.collect()
    gaussian_filter(Q_f, sigma=sig, output=Q_f)
    gc.collect()
    va,fa,na_,_ = marching_cubes(Q_f, level=thr, spacing=sp)
    ...
    # Write .bin
    bin_name = os.path.splitext(os.path.basename(npz_path))[0] + f'_s{sig}_t{thr}.bin'
