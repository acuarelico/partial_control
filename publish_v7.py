...
for npz_path in files:
    ...
    # Identical parameters for safe set and asymptotic (asymptotic ⊂ safe set).
    # Resolution increases with Δt and decreases with u₀:
    #   larger Δt → more stretching → thinner sheets
    #   smaller u₀ → thinner features
    u0_val = float(data.get('u0_ctrl', 1.0))
    dt_val = float(data.get('dt_map', 1.2))
    r_u0_vox = u0_val / h

    # Effective feature thickness shrinks with Δt
    effective_r = r_u0_vox / (dt_val / 1.2)

    # Downsample: must be < effective feature / 3
    if ds_override:
        ds = ds_override
    else:
        ds = max(1, min(int(effective_r / 3), N // 200))

    feat = effective_r / ds
    sig = min(1.5, max(0.8, feat / 3))

    thr = 0.3
    sp = (h*ds, h*ds, h*ds)

    print(f"  u₀={u0_val}, Δt={dt_val}, r_u0={r_u0_vox:.1f}vox, eff_r={effective_r:.1f}vox")
    print(f"  ds={ds} ({N//ds}³), σ={sig:.2f}, feat={feat:.1f}vox")

    # === SAFE SET ===
    Q_sm = gaussian_filter(Q[::ds,::ds,::ds].astype(np.float32), sigma=sig)
    ...
    Q_asm = gaussian_filter(Q_asymp[::ds,::ds,::ds].astype(np.float32), sigma=sig)
