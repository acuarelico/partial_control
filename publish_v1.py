...
for npz_path in files:
    ...
    # Downsample must be < feature size, otherwise sheets merge
    # Rule: ds ≤ r_u0_vox / 3 for safe set, / 2 for asymptotic
    if ds_override:
        ds_safe = ds_override
        ds_asym = ds_override
    else:
    ...
    # Smoothing: must be < feature size in downsampled coords
    # Feature in ds coords = r_u0_vox / ds
    sigma_safe = min(smooth_sigma, max(0.3, r_u0_vox / ds_safe / 6))
    sigma_asym = min(0.5, max(0.2, r_u0_vox / ds_asym / 8))

    print(f"  u₀={u0_val}, r_u0={r_u0_vox:.1f} vox")
    print(f"  Safe:  ds={ds_safe} ({N//ds_safe}³), σ={sigma_safe:.2f}")
    print(f"  Asym:  ds={ds_asym} ({N//ds_asym}³), σ={sigma_asym:.2f}")
    ...
    va,fa,na_,_ = marching_cubes(Q_asm, level=0.4, spacing=sp_asym)
    va[:,0]+=x_min; va[:,1]+=x_min; va[:,2]+=z_min
    del Q_asm, Q, Q_asymp
    print(f"  Asym raw: {len(fa):,}f")

    vs,fs,ns = decimate_mesh(vs, fs, max_faces)
    va,fa,na_ = decimate_mesh(va, fa, int(max_faces*0.8))
