...
for npz_path in files:
    ...
    # Same parameters for safe and asymptotic (asymptotic ⊂ safe set).
    # Adaptive ds based on u0 — smaller u0 needs finer grid.
    import gc
    ...
    if ds_override:
        ...
        ds = max(1, min(int(r_u0_vox / 3), N // 200))

    sig = 1.0
    thr = 0.3
    sp = (h*ds, h*ds, h*ds)

    print(f"  u₀={u0_val}, r_u0={r_u0_vox:.1f}vox, ds={ds} ({N//ds}³), σ={sig:.2f}")

    # === SAFE SET: generate → decimate → free ===
    ...
    print(f"  Safe decimated: {len(fs):,}f")
    gc.collect()

    # === ASYMPTOTIC: generate → decimate → free ===
    ...
    print(f"  Asym decimated: {len(fa):,}f")
    gc.collect()
